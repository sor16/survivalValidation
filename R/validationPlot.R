...
ValidationPlot <- function(cvList,validationMethod,evaluationFunctions=NULL,legendPosition=NULL,animate=FALSE){
    ...
    if(validationMethod=="IBS"){
        ...
        summarizedData <- FullROCData %>% group_by(threshold) %>% summarise(TPR=mean(TPR,na.rm=T),FPR=mean(FPR,na.rm=T),AUC=mean(AUC,na.rm=T),cumArea=mean(cumArea,na.rm=T)) %>% ungroup() %>% distinct(TPR,FPR)
